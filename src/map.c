...
simplet_map_t *simplet_map_new() {
  simplet_init();
  simplet_map_t *map;
  if (!(map = malloc(sizeof(*map)))) return NULL;
...
void simplet_map_free(simplet_map_t *map) {
  ...
  if (map->bounds) simplet_bounds_free(map->bounds);
  ...
  if (map->error_msg) free(map->error_msg);
...
simplet_status_t simplet_map_set_srs(simplet_map_t *map, const char *proj) {
  ...
  if (map->proj) {
    if (map->bounds) {
      ...
      map->bounds = simplet_bounds_reproject(map->bounds, (const char *)s,
                                             (const char *)proj);
      free(s);
...
double simplet_map_get_buffer(simplet_map_t *map) { return map->buffer; }
...
void simplet_map_init_matrix(simplet_map_t *map, cairo_matrix_t *mat) {
  cairo_matrix_init(mat, 1, 0, 0, -1, 0, 0);
  cairo_matrix_translate(mat, 0, map->height * -1.0);
  cairo_matrix_scale(mat, map->width / map->bounds->width,
                     map->width / map->bounds->width);
  cairo_matrix_translate(mat, -map->bounds->nw.x, -map->bounds->se.y);
}

// Set the width and height of the map.
simplet_status_t simplet_map_set_size(simplet_map_t *map, unsigned int width,
                                      unsigned int height) {
...
unsigned int simplet_map_get_width(simplet_map_t *map) { return map->width; }

// Get the height of the map
unsigned int simplet_map_get_height(simplet_map_t *map) { return map->height; }
...
simplet_status_t simplet_map_set_height(simplet_map_t *map,
                                        unsigned int height) {
  map->height = height;
  return SIMPLET_OK;
}
...
simplet_status_t simplet_map_set_bgcolor(simplet_map_t *map, const char *str) {
  free(map->bgcolor);
  if ((map->bgcolor = simplet_copy_string(str))) return SIMPLET_OK;
...
simplet_status_t simplet_map_set_bounds(simplet_map_t *map, double maxx,
...
// [tile
// coordinates](http://code.google.com/apis/maps/documentation/javascript/maptypes.html#CustomMapTypes)
simplet_status_t simplet_map_set_slippy(simplet_map_t *map, unsigned int x,
  ...
  if (!simplet_map_set_bounds(map, (x + 1) * length - origin,
                              origin - (y + 1) * length, x * length - origin,
                              origin - y * length))
    return simplet_error((simplet_errorable_t *)map, SIMPLET_OOM,
                         "out of memory setting bounds");

  return SIMPLET_OK;
}

static void *add_layer(simplet_map_t *map, simplet_layer_t *layer) {
...
simplet_vector_layer_t *simplet_map_add_vector_layer(simplet_map_t *map,
...
simplet_raster_layer_t *simplet_map_add_raster_layer(simplet_map_t *map,
                                                     const char *datastring) {
...
simplet_layer_t *simplet_map_add_layer_directly(simplet_map_t *map,
...
simplet_status_t simplet_map_is_valid(simplet_map_t *map) {
  ...
  if (!map->bounds) return SIMPLET_ERR;

  // Does it have a projection?
  if (!map->proj) return SIMPLET_ERR;

  // Does it have a height?
  if (!map->height) return SIMPLET_ERR;

  // ...and width?
  if (!map->width) return SIMPLET_ERR;

  // Does it have at least one layer?
  if (!simplet_list_head(map->layers)) return SIMPLET_ERR;
...
cairo_surface_t *simplet_map_build_surface(simplet_map_t *map) {
  // Check if the map is valid.
  if (simplet_map_is_valid(map) == SIMPLET_ERR) return NULL;
  ...
  while ((layer = simplet_list_next(iter))) {
    ...
    if (layer->type == SIMPLET_VECTOR) {
      err = simplet_vector_layer_process((simplet_vector_layer_t *)layer, map,
                                         litho, ctx);
    } else if (layer->type == SIMPLET_RASTER) {
      err = simplet_raster_layer_process((simplet_raster_layer_t *)layer, map,
                                         ctx);
    }
...
static void close_surface(cairo_surface_t *surface) {
  cairo_surface_destroy(surface);
}

// Render the map and emit a stream of chunks to closure
void simplet_map_render_to_stream(
    simplet_map_t *map, void *stream,
    cairo_status_t (*cb)(void *closure, const unsigned char *data,
                         unsigned int length)) {
  cairo_surface_t *surface;
  if (!(surface = simplet_map_build_surface(map))) return;

  if (cairo_surface_write_to_png_stream(surface, cb, stream) !=
      CAIRO_STATUS_SUCCESS)
    set_error(map, SIMPLET_CAIRO_ERR,
