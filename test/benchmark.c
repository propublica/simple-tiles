...
static void teardown_map(void *ctx) { simplet_map_free(ctx); }
...
static void teardown_list(void *ctx) { (void)ctx; }

static void initialize_map(simplet_map_t *map) {
...
static cairo_status_t stream(void *closure, const unsigned char *data,
                             unsigned int length) {
  (void)closure, (void)data, (void)length; /* suppress warnings */
  return CAIRO_STATUS_SUCCESS;
}

static void bench_empty(void *ctx) {
...
static void bench_seamless(void *ctx) {
  simplet_map_t *map = ctx;
  initialize_map(map);
...
static void bench_many_queries(void *ctx) {
  simplet_map_t *map = ctx;
  initialize_map(map);
...
static void bench_render(void *ctx) {
  simplet_map_t *map = ctx;
  initialize_map(map);
  char *data = NULL;
  simplet_map_render_to_stream(map, data, stream);
...
static void bench_text(void *ctx) {
  simplet_map_t *map = ctx;
  initialize_map(map);
  simplet_vector_layer_t *layer = simplet_list_tail(map->layers);
  simplet_query_t *query = simplet_list_tail(layer->queries);
...
static void bench_unprojected(void *ctx) {
  simplet_map_t *map = ctx;
  initialize_map(map);
...
static void bench_raster(void *ctx) {
  simplet_map_t *map = ctx;
  simplet_map_set_slippy(map, 602, 769, 11);
  simplet_map_add_raster_layer(map,
                               "./data/nyc2-rgb-pansharpened-8bit-nodata.tif");
  char *data = NULL;
  simplet_map_render_to_stream(map, data, stream);
  assert(SIMPLET_OK == simplet_map_get_status(map));
}

static void bench_raster_resample(void *ctx) {
  simplet_map_t *map = ctx;
  simplet_map_set_slippy(map, 602, 769, 11);
...
static void bench_many_raster(void *ctx) {
  simplet_map_t *map = ctx;
  simplet_map_set_slippy(map, 602, 769, 11);
  for (int i = 0; i < 10; i++)
    simplet_map_add_raster_layer(
        map, "./data/nyc2-rgb-pansharpened-8bit-nodata.tif");
  char *data = NULL;
  simplet_map_render_to_stream(map, data, stream);
  assert(SIMPLET_OK == simplet_map_get_status(map));
...
#define ITEMS 100000
static void bench_list(void *ctx) {
  simplet_list_t *list = ctx;
  int t = 1;
  for (int i = 0; i < ITEMS; i++) simplet_list_push(list, &t);
...
static double mean(double *arr, int count) { return sum(arr, count) / count; }

static double stdev(double *arr, int count) {
  double avg = mean(arr, count);
  double var = 0;
  for (int i = 0; i < count; i++) var += pow(arr[i] - avg, 2);
...
int main() {
  ...
  for (bench = (bench_wrap_t *)&benchmarks; bench->call; bench++) {
    ...
    printf("\nCompleted %i runs in %f seconds (%f r/s)\n", bench->times,
           sum(runs, bench->times), bench->times / sum(runs, bench->times));
